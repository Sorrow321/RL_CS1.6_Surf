...
var DESC = {
  ...
  'plan/cover_ep': '--plan-cover: mean coverage reward per planner primitive - for each 128 u cell the primitive added to what its episode had visited alive, C / sqrt(1 + N), N = episodes that covered the cell before (frontier-seeking; 0 on a death).',
  ...
  'plan/reward': 'The PLANNER\'s reward: its mean reward per plan (per primitive, not per episode), separate from the policy\'s. Straight-line progress toward the finish per 1000 u (with a refund at a failed end under --plan-shaping refund / refund_i, nothing taken back under plain), + the finish bonus, + end-cell novelty, + coverage.',
  ...
  'rollout/ep_rew_mean': 'The POLICY\'s (executor\'s) reward: mean episode return in TRAINING. Under a planner (--goal-planner) it is mostly the pay for following the current plan (progress along its curve), summed over the episode, so it grows with episode length. Not the planner\'s reward (see plan/reward) and not a progress metric. Not comparable across spawn distributions.',
