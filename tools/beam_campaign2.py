...
def divergence(a, b, thresh=500.0, sustain=100):
    ...
    over `thresh` u for at least `sustain` ticks (1 s) - the coordinator's
    definition of a genuinely different route segment."""
    n = min(len(a), len(b))
    if n == 0:
        return 0.0, [], 0
...
def main():
    ...
    inc_xyz = load_xyz(out_root / "beam_best.jsonl")
    print(f"incumbent: {inc_ticks} ticks = {inc_ticks / 100:.2f}s "
          f"({len(inc_xyz)} traj rows)")
        ...
        for H in h_grid:
            ...
            if failed is None and sfile.exists():
                ...
                if d.get("dnf"):
                    ...
                    mx, segs, n = divergence(
                        load_xyz(wdir / "beam_best.jsonl"), inc_xyz)
                    row["div_max_u"] = round(mx)
                    row["div_segments"] = [
                        {"from_s": s / 100.0, "to_s": e / 100.0,
