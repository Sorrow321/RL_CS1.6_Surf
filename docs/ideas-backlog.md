...
| Quantised progress coordinate (chunked arc length) | dead at N=20 | `xBIN20`: `--race-arc-bins 20`, 20 chunks of 11,584u, budget held at 100. **0 finishes in 66 episodes** against xARC's 63/102 on the same line and checkpoint, and the frontier RETREATS below the untreated opening eval (205,378 -> 197,415u). The policy unlearns line-following: off-line error at route vertex 1540 goes 202 -> 1,615u and it stops reaching the wall at all. Not discount starvation (a chunk is 4.0s, worth 4.31 discounted) - resolution: one chunk is 45x the 256u control problem at the wall. Coarsening the LINE is free (xAUTO); coarsening the POTENTIAL is fatal. |
