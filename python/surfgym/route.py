...
class ArcProgress:
    ...
    * **signed delta inside the corridor.** Paying ``arc_now - arc_prev``
    ...
    ``bins`` (``--race-arc-bins N``, 0 = off) QUANTISES the coordinate: the
    line is cut into ``N`` equal chunks and the potential becomes the CHUNK
    INDEX, ``floor(arc / (length/N))`` clipped to ``[0, N-1]``, instead of
    the arc itself. Inside a chunk the potential is flat, so there is no
    gradient at all - which is the point: it says *any pathing inside this
    chunk is equally good* and the reward stops dictating a specific route
    through it. Round 19's xLATCH found that flattening the shaping past the
    wall is what let the agent through it (0 finishes in 234 episodes became
    52/102); this applies the same move everywhere, and sweeps the density
    axis whose ends are measured - N = 1 is the goal bonus alone (xNOSHP:
    dead) and N -> infinity is continuous arc length (xARC: 63/102).

    The quantisation is a pure post-map on the potential. The anchor stays
    continuous and the order-only window and the corridor gate are untouched:
    a *global* nearest-chunk assignment would be farmable, since a global
    argmin credits a fall with up to 46,000 u where this route folds back and
    the cannonball terminal fall lands 4,703 u from the goal in straight-line
    terms - a late chunk, i.e. the agent paid for dying. ``bins = 0`` is the
    continuous path expression for expression.
    """

    def __init__(self, pts, spacing: float = DEFAULT_SPACING,
                 corridor: float = 1500.0, window: int = 16,
                 source: str = "", bins: int = 0):
        ...
        self.source = source
        # --race-arc-bins: 0 = off, and off never evaluates a branch or
        # touches an array the continuous path did not.
        self.bins = max(0, int(bins))
        self._binw = (self.length / self.bins) if self.bins > 0 else 0.0
    ...
    @property
    def pay_span(self) -> float:
        """The span of the potential one start -> finish run collects.

        The shaping scale is ``100 / pay_span``, which fixes the total
        collectible budget at 100 whatever the quantisation - the same 100
        the geodesic term gets from ``scale = 100/d0``. Continuous: the route
        length in units. Quantised into N chunks: the index runs 0 -> N-1, so
        a full run collects N-1 boundary crossings and each is worth
        ``100/(N-1)``. Holding the budget fixed is what keeps this arm a
        one-variable change - the racing-beats-quitting constraint is that
        shaping income exceeds ``time_pen`` (measured cliff 0.0125/tick), and
        a scale derived from anything else would move that too."""
        if self.bins <= 0:
            return self.length
        return float(max(self.bins - 1, 1))

    def _bin(self, arc):
        """Arc coordinate -> chunk index (float, for a plain subtraction)."""
        return np.clip(np.floor(arc / self._binw), 0.0, self.bins - 1.0)
    ...
    def describe(self) -> str:
        return (f"arc route {Path(self.source).name or '<memory>'}: "
                ...
                + ("" if self.bins <= 0 else
                   f", QUANTISED into {self.bins} chunks of "
                   f"{self._binw:,.0f}u (potential = chunk index 0..."
                   f"{self.bins - 1}, flat inside a chunk)"))
    ...
    def advance(self, origin):
        ...
        if self.bins > 0:
            # the potential is the CHUNK INDEX, so the paid delta is a
            # difference of indices: 0 for any motion that stays in one
            # chunk, +/-1 on a crossing. The anchor below stays CONTINUOUS -
            # quantising it too would make the order-only window and the
            # corridor gate operate on a 11,584u grid, and the telescoping
            # would no longer be exact.
            delta = np.where(inside, self._bin(arc) - self._bin(self.arc), 0.0)
        else:
