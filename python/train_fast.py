...
def main() -> None:
    ...
    ap.add_argument("--plan-corridor", type=float, default=None,
                    help="--goal-planner learned (walking): the completion "
                         "judge's tracking tolerance in u - a plan counts as "
    ...
    ap.add_argument("--plan-lturn", action="store_true",
                    help="--goal-planner learned (walking): add SHARP-turn "
                         ...
                         "3/4 of the shape, then a +-45 / +-90 deg corner "
                         "(16 x 12 more shapes). Stored in the planner's spec")
    ...
    ap.add_argument("--plan-sil", type=float, default=None,
                    help="--goal-planner primlearn: self-imitation learning (Oh et al. 2018) for "
                         "the planner - the decisions of FINISHED episodes are kept with their "
    ...
    ap.add_argument("--plan-sil-uniform", type=int, default=None, choices=(0, 1),
                    help="--plan-sil: 1 = the UNIFORM opening primitives (--plan-uniform, never a "
                         ...
                         "finishes, with their own progress + the discounted return of the "
                         "planner's primitives that followed - the explorer's finds are imitated. "
                         "Default 0; ckpt restores")
    ...
    ap.add_argument("--int-move-gate", action="store_true", default=None,
                    help="novelty is paid only when the POSITION cell changes; the "
    ...
    if PLPLAN:
        ...
        if float(args.plan_sil or 0.0) > 0.0:
    ...
    if LPLAN:
        meta["config"].update({_k: getattr(args, _k) for _k in _lp_knobs})
        if args.plan_corridor is not None:
    ...
    if args.goals:
        ...
        if LPLAN:
            ...
            if (args.plan_corridor is not None or args.plan_lturn
                ...
                # = no spec, the call that shipped
                from surfgym.goallearn import _spec_default as _lsd
                _ws = _lsd()
                if args.plan_corridor is not None:
                    _ws["corridor"] = float(args.plan_corridor)
                if args.plan_lturn:
